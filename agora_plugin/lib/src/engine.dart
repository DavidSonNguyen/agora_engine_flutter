...
class AgoraEngine {
  static const MethodChannel _channel =
      const MethodChannel('agora_method_channel');
  static const EventChannel _eventChannel =
      const EventChannel('agora_event_channel');

//  agora_rtc_engine_event_channel
  ...
  static void Function(String userInfo, int uid) onUpdatedUserInfo;
  ...
  static Function onLeaveChannel;
  ...
  /// 0. Leave the channel: When the user/host leaves the channel, the user/host sends a goodbye message. When this message is received, the SDK determines that the user/host leaves the channel.
  /// 1. Drop offline: When no data packet of the user or host is received for a certain period of time (20 seconds for the communication profile, and more for the live broadcast profile), the SDK assumes that the user/host drops offline. A poor network connection may lead to false detections, so Agora recommends using the signaling system for reliable offline detection.
  ...
  static Function onRequestToken;
  ...
  static Function onMediaEngineLoadSuccess;

  /// Occurs when the media engine starts.
  static Function onMediaEngineStartCallSuccess;
  ...
  static Future<void> setClientRole(ClientRole role) async {
    int roleValue = intFromClientRole(role);
  ...
  /// stringuid userAccountuserAccountssdvs
  /// registerLocalUserAccount
  static Future<bool> registerLocalUserAccount(
      String appId, String userAccount) async {
  ...
  static Future<bool> joinChannelByUserAccount(
      String userAccount, String token, String channelId) async {
    final bool result =
    ...
    if (!result) {
      await leaveChannel();
      await joinChannelByUserAccount(userAccount, token, channelId);
    }
    print('joinChannelByUserAccount: $result');
    return result;
  }
  ...
  static Future<Map<dynamic, dynamic>> getUserInfoByUserAccount(
      String userAccount) async {
    final t = await _channel
        .invokeMethod('getUserInfoByUserAccount', {"userAccount": userAccount});
    return {
      'uid': t['uid'],
  ...
  static Future<Map<dynamic, dynamic>> getUserInfoByUid(int uid) async {
    final t = await _channel.invokeMethod('getUserInfoByUid', {"uid": uid});
    return {
      'uid': t['uid'],
      'userAccount': t['userAccount'] ?? '',
    };
  ...
  static Future<void> enableLocalAudio(bool enabled) async {
    await _channel
        .invokeMethod('enableLocalAudio', {'enabled': enabled ?? true});
  }
  ...
  // setParameters
  static Future<int> setParameters(EngineParams param) async {
    final int res = await _channel
        .invokeMethod('setParameters', {"params": param.toString()});
    return res;
  }
    ...
    switch (map['event']) {
      ...
      case 'onClientRoleChanged':
        if (onClientRoleChanged != null) {
          ClientRole oldRole = clientRoleFromInt(map['oldRole']);
          ClientRole newRole = clientRoleFromInt(map['newRole']);
      ...
      case 'onUpdatedUserInfo':
        if (onUpdatedUserInfo != null) {
          onUpdatedUserInfo(map['userAccount'], map['uid']);
        }
        break;
  ...
  static ClientRole clientRoleFromInt(int value) {
    switch (value) {
      case 1:
  ...
  static int intFromClientRole(ClientRole role) {
    switch (role) {
